...
def load_config():
    ...
    config = {
        "clip": {"enabled": True, "copy_mode": "path", "notice": {"toast": {"enabled": True}, "sound": {"enabled": True}}},
        "output": {
            "enabled": True,
...
def main():
    ...
    if config["clip"]["enabled"]:
        try:
            copy_mode = config["clip"].get("copy_mode", "text")
            if copy_mode == "path" and filepath:
                # Copy the absolute path to the clipboard
                pyperclip.copy(str(filepath.absolute()))
                print(f"File path has been copied to clipboard: {filepath.absolute()}")
            else:
                # Fallback to copying markdown text
